...
### ✨ UI/UX改进 (UI/UX Enhancement)

#### 增强Refinement模式符合SAM2官方最佳实践

**背景：**
根据SAM2官方文档（`video_predictor_example.ipynb`），refinement过程推荐使用点击提示而非边界框，因为：
- 边界框在tracking开始后添加时，SAM2可能无法很好地整合
- 点击提示更适合精细化修正
...
2. **阻止Refinement模式下的Box绘制** (`micro_tracker/components/video_widgets.py`)
   - 修正模式下尝试绘制边界框时显示警告提示
   - 强制用户使用点击提示
   - 提示信息："⚠️ 修正模式下不能绘制新的边界框，请使用点击提示（符合SAM2官方规范）"

3. **标注列表显示优化** (`micro_tracker/ui/annotation_manager.py`)
   - 支持显示纯点击标注（无边界框的帧）
   - 合并bbox和refinement数据源
   - 正确统计包含点击标注的对象数量
...
**用户体验提升：**
- ✅ 减少误操作：自动模式切换 + UI约束
- ✅ 符合最佳实践：引导用户使用SAM2推荐方式
- ✅ 透明反馈：清晰的警告和提示信息
- ✅ 完整支持：纯点击标注也能在UI中正常显示

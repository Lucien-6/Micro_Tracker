...
Phase 2采用了**统一的多帧处理架构**：

- **无需模式切换**：单帧和多帧使用相同处理路径
- **单帧 = 多帧特例**：只有1个标注帧的多帧处理
- **代码简洁**：移除70+行冗余代码
- **易于维护**：单一处理逻辑，无分支判断

```
旧架构（Phase 1）:
  if 单帧: process_video_main()
  if 多帧: process_video_multiframe()  ← 两套代码！

新架构（Phase 2 统一）:
  统一: process_video_multiframe()      ← 一套代码！
  单帧自动视为 {0: bbox_list}
```

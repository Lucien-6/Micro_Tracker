...
class FilterMaskThread(QThread):
    """Background mask filtering: load PNGs, per-object geometry/kinematics, criteria, visualization.

    If ``precomputed`` is set (same masks + fps + μm/pixel as cached analysis), skips disk load
    and geometry/kinematics; only runs filter + preview build. Controller supplies precomputed.
    """
    ...
    def __init__(self, masks_dir, fps=1.0, um_per_pixel=1.0, filter_params=None, precomputed=None):
        super().__init__()
        self.masks_dir = masks_dir
        self.fps = fps
        self.um_per_pixel = um_per_pixel
        self.filter_params = filter_params or {}
        # Optional dict from FilterController: masks_data, object_ids, object_data, mask_filenames
        self.precomputed = precomputed
        self.is_running = True
        # Set only after full load+analysis; copied into controller cache on success (None if precomputed path)
        self._analysis_cache_for_controller = None
    ...
    def run(self):
        try:
            if self.precomputed is not None:
                pcm = self.precomputed
                self.progress_update.emit(
                    "已缓存掩膜与分析数据，跳过磁盘加载与几何/运动计算，直接应用筛选条件…"
                )
                self.progress_percent.emit(30)
                masks_data = pcm["masks_data"]
                object_ids = pcm["object_ids"]
                object_data = pcm["object_data"]
                mask_files = list(pcm["mask_filenames"])
                if not masks_data:
                    raise Exception("缓存的掩膜数据为空")
                if len(mask_files) != len(masks_data):
                    raise Exception("缓存元数据不一致（掩膜文件名数量与帧数不符）")
                self.all_masks = mask_files
                self.original_masks = masks_data
                self.total_objects = len(object_ids)
                self._analysis_cache_for_controller = None
                self._apply_filters_and_visualize(masks_data, object_data, object_ids)
                return
            ...
            # 逐帧逐对象：几何与运动统计
            self.progress_update.emit("正在计算对象几何与运动属性...")
            ...
            self._analysis_cache_for_controller = {
                "masks_data": masks_data,
                "object_ids": set(object_ids),
                "object_data": object_data,
                "mask_filenames": tuple(mask_files),
            }
            self._apply_filters_and_visualize(masks_data, object_data, object_ids)
    ...
    def _apply_filters_and_visualize(self, masks_data, object_data, object_ids):
        """Apply filter_params and render preview frames (same code path after full run or cache hit)."""
        total_frames = len(masks_data)
        self.object_filter_results = {}
